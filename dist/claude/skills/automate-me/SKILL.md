...
Locate the active workspace's transcripts before fanning out. Use only Claude Code's documented project transcript under `~/.claude/projects/<project-key>/<session>.jsonl`, including that session's `<session-id>/subagents/` records for the active project. Do not scan undocumented provider storage or unrelated projects.

Survey recent records within that scope for recurring patterns. Invoke `pstack-role-how-explorer` across 3 supported slices from Claude Code's documented project transcript under `~/.claude/projects/<project-key>/<session>.jsonl`, including that session's `<session-id>/subagents/` records, divided by native timestamps and IDs. Give each explorer only its slice, the signals below, and a request for a short structured list with native evidence pointers. Do not infer filenames, scan undocumented storage, or cross project boundaries. Default signals worth hunting:
