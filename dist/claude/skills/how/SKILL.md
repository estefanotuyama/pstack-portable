...
- Native profile: `pstack-role-how-explorer` (read-only)
...
Spawn a single native subagent that explores and explains in one pass:

- Native profile: `pstack-role-how-explainer` (read-only)
...
Once all explorers return, spawn a single native subagent to synthesize their findings into one coherent explanation:
...
After the explanation is complete, spawn `pstack-role-how-critic-1` through `pstack-role-how-critic-4` as four independent read-only critics, all in a single message.
