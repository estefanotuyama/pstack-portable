...
4. Copy the skeleton below into the plan file and fill every placeholder. Unless the operator names a path, write the file under `~/.pstack/state/claude/<project-slug>/docs/`. Keep every heading and every sub-block in the order shown. One section per PR. One PR is one change with its own evidence (the **sequence-verifiable-units** principle skill). Name the execution playbook in **How to read this**. Pick between `playbooks/autopilot-full.md` and `playbooks/autopilot-stack.md` per the rule at the end of `playbooks/autopilot-stack.md`. A standing program takes `playbooks/orchestrate.md`.
...
6. Run `node ~/.pstack/providers/claude/skills/poteto-mode/scripts/check-plan.mjs <plan.md>` and fix every line it prints (the **encode-lessons-in-structure** principle skill). It enforces the skeleton's shape, the verification rule in every verification block, and the punctuation rules.
...
- [ ] Read these from trunk at program start. Re-read them at every tick.
  - [ ] `~/.pstack/providers/claude/skills/poteto-mode/playbooks/<execution playbook>.md`
  - [ ] `~/.pstack/providers/claude/skills/swarm/SKILL.md`
  - [ ] `git show origin/main:<control skill path>`
  - [ ] `~/.pstack/providers/claude/skills/poteto-mode/playbooks/opening-a-pr.md`
  - [ ] `~/.pstack/providers/claude/skills/<each other leaf skill the program uses>`
- [ ] Arm the 30-minute audit tick. In a local session, a real terminal `/loop`. In a local coordinator, a checkpoint watcher wake chain. Never leave the cadence to memory.
