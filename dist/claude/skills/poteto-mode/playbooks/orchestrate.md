...
Create `~/.pstack/state/claude/<project-slug>/orchestrate/` as the durable store. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `bun ~/.pstack/providers/claude/skills/poteto-mode/scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it with Claude Code's native `/loop` and a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
