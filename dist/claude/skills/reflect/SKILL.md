...
Resolve this conversation through Claude Code's documented project transcript under `~/.claude/projects/<project-key>/<session>.jsonl`, including that session's `<session-id>/subagents/` records. Use only the active project. If the provider cannot expose the record, write a tight digest from the current conversation and pass that instead. Do not scrape undocumented storage.
...
Pass each template verbatim, substituting the supported session record or digest where marked. Reviewers return findings in the `Agent` response body.
...
Before editing, classify the target. If it is under `~/.pstack/providers/claude/`, `dist/claude/`, or another installed/generated pstack path, do not edit it directly: propose the change against the canonical portable repository's generator or upstream snapshot and regenerate after review. A standalone user-authored skill at .claude/skills/ or ~/.claude/skills/ may be edited in its native scope.
