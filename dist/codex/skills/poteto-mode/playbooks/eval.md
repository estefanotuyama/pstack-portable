...
6. **Verify the chain from the supported session record, not self-report.** Resolve each run through the supported task/thread reader or an explicit task export/state capsule. If any record is unavailable, the evidence-critical eval fails closed. Never scrape undocumented storage. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
