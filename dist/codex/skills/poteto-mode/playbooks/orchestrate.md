...
- Every spawn and every fresh respawn carries the standing orders verbatim.
...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, observed, and freshly respawned only through the provider's native subagent capability. State reads and writes go through `~/.pstack/providers/codex/skills/poteto-mode/scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (native nested spawning may be used only when the active runtime confirms it). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
...
Create `~/.pstack/state/codex/<project-slug>/orchestrate/` as the durable store. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `bun ~/.pstack/providers/codex/skills/poteto-mode/scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.

- `preferences.md` is the standing-orders register: numbered lines, one constraint each (model policy, stack shape and count, verification bar, forbidden paths, escalation policy). Paste it verbatim into every spawn and every fresh respawn; directives decay across fresh respawns, and each dropped one costs a human turn. When you catch yourself restating an instruction, append the line before you act (principle-encode-lessons-in-structure).
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe request_user_input state.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Local spawns may reference the standing-orders file by store path; verbatim paste is for every fresh spawn and respawn.

A sub-coordinator brief adds its track boundary and unit list, its exact requested fan-out, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm `/goal` and use a Desktop heartbeat or local CLI/IDE watcher with a long fallback interval), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run only in the serialized stacker worktree; never run two stack mutations concurrently.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run: a worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one ephemeral worker or worktree when it disappears was never done.

#### Liveness and failure

- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the provider's native agent-status surface or persisted branch, PR, ledger, and store artifacts. Transcript mtime is not liveness.
...
- Retry by mode: cap-hit or oom, respawn with smaller scope; network-drop, retry as-is; tool-error, retry the same semantic profile; change the configured mapping only through setup-pstack with an explicit decision; unknown, retry once. Two retries, then abandon the unit and replan around it.
