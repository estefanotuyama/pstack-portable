...
Resolve chat history through the supported task/thread reader or an explicit task export/state capsule. Scope it to the active project and requested time window. If that surface is unavailable, disclose the gap and continue from live state and the shared record; never scrape undocumented storage.
